...
public class CameraToMpegTest extends AndroidTestCase {
    ...
    private static class CameraToMpegWrapper implements Runnable {
        ...
        public void run() {
            try {
                mTest.encodeCameraToMpeg();  // 在这里跑一段时间   使用Camera MediaCodec Surface进行录像   最后线程退出
            } catch (Throwable th) {
                mThrowable = th;
            }
            // 到这里 测试程序完毕
    ...
    private void encodeCameraToMpeg() {
        ...
        try {
            ...
            prepareSurfaceTexture(); // 从SurfaceManager获得一个SurfaceTexture给到Camera preview
            ...
            while (System.nanoTime() < desiredEnd) {
            	...
            	/* 获得解码的输出 并且给到muxer
            	 * drainEncoder 会取出所有的编码输出的NAL 等待10ms没有编码输出的话 ，就返回
            	*/
                drainEncoder(false);
                ...
                // argument.
                mStManager.awaitNewImage(); // 等待摄像头的一个预览数据    setPreviewTexture
                
                // 应用程序通过OpenGL API进行绘制，一帧完成之后，调用eglSwapBuffers(EGLDisplay dpy, EGLContext ctx)来显示
                // 这样就会画到 mInputSurface 上
                mStManager.drawImage();

                //  把SurfaceTexture的时间戳作为MediaMuxer的pts 
                ...
                /* 提交到解码器   EGL14.eglSwapBuffers(mEGLDisplay, mEGLSurface); 会阻塞  如果输入已经满了
                	这会导致不好的结果 如果他一直阻塞到我们获取一个output buffer(目前代码没法做 因为我们阻塞在这里)
                	
                	只要我们在提供更多输入的之前 我们把编码器输出都取出来 系统会保证我们能提供没有阻塞地 更多帧输入  
                */
                ...
                mInputSurface.swapBuffers(); // OpenGL ES API已经把图片给到MediaCodec.inputSurface swapBuffers之后MediaCodec就有一帧数据需要解码了
            }
    ...
    private void prepareEncoder(int width, int height, int bitRate) {
        ...
        mInputSurface = new CodecInputSurface(mEncoder.createInputSurface());// 获得一个Surface作为编码器输入  而不是用dequeueInputBuffer
        mEncoder.start();
    ...
    private static class CodecInputSurface {
        ...
        private void eglSetup() {
            mEGLDisplay = EGL14.eglGetDisplay(EGL14.EGL_DEFAULT_DISPLAY);  // 获取显示设备   默认对应手机主屏幕
            ...
            int[] attribList = {
                    ...
                    EGL14.EGL_RENDERABLE_TYPE, EGL14.EGL_OPENGL_ES2_BIT,//指定渲染api类别 OPENGL_ES2
            ...
            */
             ...
             * 创建一个window surface 并且关联到给定的Surface 
             * */
    ...
    private static class SurfaceTextureManager
        ...
        public SurfaceTextureManager() {
            ...
            // 创建一个SurfaceTexture来把一个图片 流传输给 指定的OpenGL texture
        ...
        public void awaitNewImage() {
            ...
            synchronized (mFrameSyncObject) {
                ...
                mFrameAvailable = false; // 告诉 onFrameAvailable 已经处理
            }

            // Latch the data.
            mTextureRender.checkGlError("before updateTexImage");
            mSurfaceTexture.updateTexImage(); // 把SurfaceTexture的数据给到了OpenGL texture更新 ??
        }
        ...
        public void drawImage() {
            mTextureRender.drawFrame(mSurfaceTexture); // 从SurfaceTextrue中的数据 画到 EGL surface.
        }
        ...
        private static class STextureRender {
            ...
            public void surfaceCreated() {
                ...
                GLES20.glGenTextures(1, textures, 0); // GenTextures  产生一个GLES20纹理 

                mTextureID = textures[0];
                GLES20.glBindTexture(GLES11Ext.GL_TEXTURE_EXTERNAL_OES, mTextureID); // BindTexture 绑定指定纹理
